...
export const updateById = async (id, book) => {
  try {
    const { titulo, autor, editorial, anio_publicacion, genero, existencias } = book;
    const query = "UPDATE libro SET titulo = ?, autor = ?, editorial = ?, anio_publicacion = ?, genero = ?, existencias = ? WHERE id_libro = ?";
...
export const updateStockById = async (id, existencias) => {
  try {
    ...
    throw new Error(`Could not update stock for book with id ${id} in the database.`);
  }
};
