...
export const create = async (body) => {
  try {
    ...
    const { nombre, apellido, mail, contrasenia, id_rol } = body;

    const hassPass = encriptador.hashSync(contrasenia, 12);
    const query =
...
//metodo util para el login

export const findByMail = async (mail) => {
  try {
    const consulta = `SELECT mail, contrasenia, nombre, apellido FROM usuario WHERE mail = ?`;
    const [rows] = await pool.execute(consulta, [mail]);
